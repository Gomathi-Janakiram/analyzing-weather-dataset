...
def categorical(df):
    cate=df.select_dtypes(include='object').columns.tolist()
    return cate
...
def clear(df,col,val):
    valuecount=df[df[col]==val].count()
    return valuecount[1]
...
def instances_based_condition(df,col1,val1,col2,val2):
    condition1=df[col1]>val1
    condition2=df[col2]==val2
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    aggregated_value=pd.pivot_table(df,index=df[date_col].dt.month,values=agg_col,aggfunc={agg_col:aggregate[agg]}) 
...
print("categorical_columns",categorical(weather))
print("numerical_columns",numerical(weather))
...
print("valuecounts",clear(weather,'Weather','Rain Showers'))
...
mean_farenheit=convert(weather,'Temp (C)')
